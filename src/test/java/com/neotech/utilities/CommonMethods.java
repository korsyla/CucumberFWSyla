...
//---------------------------------------- COMMON METHOD #9 -------------------------------------------------------------
	 ...
	 * @param text
	 */
	public static void sendTextAlert(String text) //Here the user(me), is sending the text that I want 
	{
		try 
		{
...
//---------------------------------------- COMMON METHOD #14 -------------------------------------------------------------
	...
	/**
	 * This Method creates a wait object (return wait) ONLY!  
	 * And we use its method name getWaitObject() to Link it with waitForClickability & waitForVisability Methods
	 * @return
	 */
...
//---------------------------------------- COMMON METHOD #23 -------------------------------------------------------------
	 ...
	 * Before using this method we have to put select drop down elements in a list/than  
	 * Than below it, we can call this method
	 * List<WebElement> genderOptions = personalDetails.genderOptions;
	 *  Than We call THIS method  -> clickOnDropDownElement(genderOptions, gender);
	 *  we can call it like this -> clickOnDropDownElement(personalDetails.licsExpYearOptions, year);
	 */
	public static void clickOnDropDownElement(List<WebElement> optionsDDList, String examplesValue) 
	{
		for(WebElement optionDDElement : optionsDDList) 
	    {
	    	if(optionDDElement.getText().equals(examplesValue)) 
	    	{
	    		click(optionDDElement);
	    		break;
	    	}
	    }
	}
